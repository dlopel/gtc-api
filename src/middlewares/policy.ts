...
import { isIdUnique } from '../services/policy'
...
/**Only one image to upload (policy image is required)*/
export const uploadPolicyFile = (
    req: Request,
    res: Response,
    next: NextFunction) => {
...
export const uploadPolicyFileToCloudinary = async (
    ...
    try {
        ...
        if (req.file) {
            ...
            //if exists previous image path, delete from cloudinary
            // const previousImagePath = await getImagePathById(req.body.id)
            // if (previousImagePath) {
            //     destroyCloudinaryImageByPath(previousImagePath)
            // }
        } else {
...
export const isPolicyValid = async (
    req: Request<unknown, unknown, PolicyStorage>,
    res: Response,
    next: NextFunction
) => {
    ...
    try {
        ...
        let error = false
        try {
            isUnique = await isIdUnique(policy.policy.id)
        } catch (er) {
            error = true
            await deleteLocallyUploadedFiles({ image: [req.file] })
            throw er
        ...
        next(new GenericException(
            'Error validating Policy and unique constraint of the Id',
            { policy: req.body },
            error.stack
        ))
    }
